#!/usr/bin/env python3
"""
FAST Toxic Comment Detector - FIXED
Properly limits samples per language split
"""
...
# ============================================================
# LOAD DATA - FIXED: Limits per language split BEFORE combining
# ============================================================

def load_data_fast(sample_size=1000):
    """Load TextDetox with properly limited samples"""
    print(f"\n{'='*60}")
    print(f"Loading TextDetox (target: ~{sample_size} total samples)")
    print(f"{'='*60}")
    
    start = time.time()
    
    ds = load_dataset("textdetox/multilingual_toxicity_dataset", "default")
    num_splits = len(ds.keys())
    
    # Distribute samples evenly across languages
    per_lang = max(1, sample_size // num_splits)
    print(f"Languages found: {num_splits}")
    print(f"Max per language: {per_lang}")
    ...
    for split_name in ds.keys():
        ...
        # LIMIT EACH LANGUAGE BEFORE combining
        original_len = len(df)
        if len(df) > per_lang:
            df = df.sample(n=per_lang, random_state=42)
        
        dfs.append(df)
        print(f"  {split_name}: {len(df)}/{original_len} samples")
    ...
    print(f"\n{'='*60}")
    print(f"Loaded: {len(combined)} total samples in {elapsed:.1f}s")
...
class FastFeatureExtractor:
    ...
    def extract(self, texts, desc="Extracting"):
        ...
        for i in range(0, total, BATCH_SIZE):
            ...
            print(f"  {progress}/{total} ({pct:.0f}%) | {elapsed:.0f}s | ~{eta:.0f}s remaining")
...
def main():
    ...
    print(f"{'='*60}")
    print("TOXIC COMMENT DETECTOR - FIXED")
    print(f"{'='*60}")
    
    # CHANGE THIS: 500=very fast, 1000=balanced, 2000=more accurate
